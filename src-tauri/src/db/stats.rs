...
pub(super) fn get_statistics(conn: &Connection) -> Result<Statistics, DbError> {
    ...
    let avg_wpm_raw: Option<f64> =
        conn.query_row("SELECT AVG(wpm) FROM entries WHERE wpm IS NOT NULL", [], |r| {
            r.get(0)
        })?;
...
/// the Python `STOPWORDS` set in `history.py`.
static STOPWORDS: once_cell_set::StopwordSet = once_cell_set::StopwordSet;

/// Tiny zero-dependency wrapper providing `.contains(&str)` over a static slice,
/// so we don't pull in `once_cell` / `lazy_static` just for a stopword set.
mod once_cell_set {
    /// The stopword list (deduplicated where the Python source had duplicates,
    /// e.g. "again"/"once"/"just"/"no" — `set` membership is identical).
    const WORDS: &[&str] = &[
        "a", "an", "the", "and", "or", "but", "in", "on", "at", "to", "for", "of", "with", "by",
        "from", "as", "is", "was", "are", "were", "been", "be", "have", "has", "had", "do",
        "does", "did", "will", "would", "could", "should", "may", "might", "must", "shall", "can",
        "need", "dare", "ought", "used", "i", "you", "he", "she", "it", "we", "they", "me", "him",
        ...
        "there", "then", "once", "if", "because", "until", "while", "about", "into", "through",
        "during", "before", "after", "above", "below", "between", "under", "again", "further",
        "any", "up", "down", "out", "off", "over", "going", "gonna", "like", "okay", "ok", "yeah",
        "yes", "um", "uh", "ah", "oh", "well", "right", "actually", "basically", "really", "thing",
        "things", "something", "anything", "everything",
    ];
