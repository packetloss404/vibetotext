...
export function animateNebula(dt, t, cameraPosition) {
    ...
    for (let i = sentenceSprites.length - 1; i >= 0; i--) {
        ...
        if (flyIn) {
            ...
            ws.position.x = ws.userData.baseX + Math.sin(t * 0.1 + phase) * 0.005;
            ws.position.y = ws.userData.baseY + Math.sin(t * 0.15 + phase * 1.3) * 0.003;
            ws.position.z = ws.userData.baseZ + Math.cos(t * 0.12 + phase * 0.7) * 0.005;
